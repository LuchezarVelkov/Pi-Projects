...
buzzer = 18     # GPIO number of Buzzer

#Setup GPIO Direction
GPIO.setup(touch, GPIO.IN)      # GPIO.setup(touch, GPIO.IN, GPIO.PUD_DOWN)
GPIO.setup(green_led, GPIO.OUT) # Set pin to be output !
GPIO.output(green_led, 0)       # Set pin to be in low state !
GPIO.setup(red_led, GPIO.OUT)   # Set pin to be output !
GPIO.output(red_led, 0)         # Set pin to be in low state !
GPIO.setup(yelow_led, GPIO.OUT) # Set pin to be output !
GPIO.output(yelow_led, 0)       # Set pin to be in low state !
GPIO.setup(buzzer, GPIO.OUT)    # Set pin to be output !
GPIO.output(buzzer, 0)          # Set pin to be in low state !

#Create function blink.
def blink(pin):                          # Create function named blink for leds
...
#Create function acept for buzzer.
def acept(pin):                          # Create function for buzzer acept sound 
        GPIO.output(pin,GPIO.HIGH)       #
        time.sleep(0.2)                  #
        GPIO.output(pin,GPIO.LOW)        #
        time.sleep(0.1)                  #
...
#Create function denine for buzzer.
def denine(pin):                         # Create function for buzzer denine sound 
        GPIO.output(pin,GPIO.HIGH)       #
        time.sleep(1)                    #
        GPIO.output(pin,GPIO.LOW)        #
        time.sleep(0.1)                  #
        ...
        try:
                if GPIO.input(touch) == 0: # if reader is touched rund srcipt
                        ...
                        if ( positionNumber == -1 ): # -1 is not recognised
                                for i in range(0,1):               # How time to run function
                                        denine(buzzer)             # Wath function to run
                                #os.system('mpg321 lock.mp3 -q')   # Play song (mp3) if not recognised
                                ...
                                for i in range(0,1):                # How time to run function
                                        acept(buzzer)               # Wath function to run
                                #os.system('mpg321 unlock.mp3 -q')  # Play song (mp3)when recognised
